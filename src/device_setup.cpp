    ...
    snd_pcm_format_t &stream_format)
{
    ...
    int err = snd_pcm_open(&handles.capture, capture_device.c_str(), SND_PCM_STREAM_CAPTURE, 0);
    if (err < 0)
    {
        std::cerr << "open capture " << capture_device << " failed: " << snd_strerror(err) << "\n";
        return 1;
    }
    handles.capture_device_id = capture_device;

    err = snd_pcm_open(&handles.playback, playback_device.c_str(), SND_PCM_STREAM_PLAYBACK, 0);
    if (err < 0)
    {
        std::cerr << "open playback " << playback_device << " failed: " << snd_strerror(err) << "\n";
        snd_pcm_close(handles.capture);
        handles.capture = nullptr;
        return 1;
    }
    handles.playback_device_id = playback_device;
    ...
    {
        if (format_supported(handles.capture, format_candidate) && format_supported(handles.playback, format_candidate))
        {
